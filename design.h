...
class Design {
 public:
  ...
  /*
   * Searches for 0 in the visited map
   */
  bool find0(map<string, int> m);

  /*
   * Helper to implement recursion on topological sort
   */
   void dfs_visit(string node, vector<Net *> *sorted);
 ...
 private:
  ...
  vector<string> pis;
  vector<string> pos;        
  map<string, int> visited;     //name of gate -- visited situation
                  // 0 - not visited
                  // 1 - visited but unresolved
                  // 2 - visited and resolved
